...
namespace Proyectile.Server.Entitys.Services
{
    public class UsuarioServices
    {
        ...
        public async Task<Usuario?> TraerUsuarioEmail(string email, string password)
        {
        ...
        public async Task<Usuario?> DesasignarProyecto(Usuario? user, Proyecto proyecto)
        {
            if (user != null)
            {
                var up = await _context.UsuariosProyectos
                  .Where(up => up.ProyectoID == proyecto.Id && up.UsuarioID == user.Id)
                  .FirstOrDefaultAsync();
                if (up != null)
                {
        ...
        public async Task<Usuario?> DesasignarTarea(Usuario? user, Tarea tarea)
        {
            if (user != null)
            {
                var usuarioTarea = await _context.UsuariosTareas
                  .Where(ut => ut.TareaID == tarea.Id && ut.UsuarioID == user.Id)
                  .FirstOrDefaultAsync();
        ...
        public async Task<Usuario> BuscarUsuarioPorCredencialesAsync(string email, string contraseña)
        {
            ...
            if(usuario!=null){
                ...
                var user_aux = await TraerUsuarioId(usuario.getId());
                if(user_aux != null)
                {
                    return user_aux;
                }
